...
while 1:
    ...
    if isinstance(cur_tile, tuple):
                    ...
                    for choicex in (cur_tile[0] + 1, MS.columns, 1), (MS.columns-2, -1, -1), (1, MS.columns, 1):
                        for x2 in range(*choicex):
                            if MS.matrix[y2][x2-choicex[2]] == 0 or MS.matrix[y2 - choicey[2]][x2] == 0:
                                if clear(x2, y2, MS.matrix[y2][x2], tiles[y2][x2]):
                                    sum_of_closed_tiles -= 1
        ...
        # lose condition
            ...
            game_is_going = 0
            clear(cur_tile[0], cur_tile[1], -1, 3)
            for y, row in enumerate(tiles):
                for x, status in enumerate(row):
                    if 1 <= status <= 2:
                        clear(x, y, MS.matrix[y][x], 3)
                    if 5 <= status <= 6:
                        clear(x, y, MS.matrix[y][x], 4)
        else:
            clear(cur_tile[0], cur_tile[1], cur_tile[3], 1)
            sum_of_closed_tiles -= 1
